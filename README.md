...
- **읽기·텍스트 추출** — hwp/hwpx → plain / markdown / **HTML** / JSON(전체 IR). 표·이미지·머리말/
  꼬리말·미해석 레코드까지 보존하며 파싱한다. 제목/지은이/주제/키워드 **문서 메타데이터**도 추출.
- **포맷 변환** — hwp ↔ hwpx, hwp/hwpx → markdown / **HTML** / **PDF**(텍스트 선택가능) / **ODT** / JSON(IR).
  공용 문서 모델(IR)을 경유한 변환.
- **PDF / 이미지 렌더링** — hwp/hwpx → **PDF(벡터·텍스트 선택가능·폰트 서브셋·이미지 임베드)** / **PNG** /
  **SVG**. 파일에 저장된 줄 배치(PARA_LINE_SEG)를 우선 사용하고, 없으면 자체 줄바꿈으로 보정한다.
  표·이미지·머리말/꼬리말·밑줄/취소선·글상자·도형(사각형/타원/선/placeholder)을 그린다.
- **문서 쓰기 (hwp 바이너리 포함)** — hwpx 패키지 쓰기와 **HWP 5.0 바이너리(CFB) 쓰기**를 모두 구현.
  hwp 출신·무수정 문서는 표·이미지·도형·책갈피를 포함해도 압축 해제 스트림 기준 **바이트 동일 왕복**까지
  보장한다(전체 fixture 게이트). 제목/지은이 메타데이터(요약 정보/OPF)도 쓴다.
- **템플릿 채우기·검증** — `{{name}}` 슬롯 스캔(`slots`)과 패키지·미리보기 보존 치환(`fill`, hwpx),
  구조 검증(`validate`).
- **AI 편집** — IR을 JSON으로 내보내 고치고 되쓰는 read→edit→rewrite 왕복. 텍스트 치환, 표 셀 설정,
  누름틀/필드 채우기, 메타데이터 설정, 메모 추가를 이미지·서식·미해석 레코드를 보존한 채 적용한다.
- **MCP 서버** — 의존성 없는(serde_json만) stdio MCP 서버로 **11개 도구**를 노출. 에이전트가 문서를
  읽고·렌더해서 직접 보고·편집·변환한다.

## 현재 범위와 한계

- **구현 완료**: hwp/hwpx 읽기, hwpx 쓰기, **hwp 바이너리 쓰기**(convert hwpx→hwp, new→hwp,
  edit→hwp 포함), markdown/HTML/**PDF**/**ODT**/JSON 변환, PNG/SVG/**PDF** 렌더링, 렌더 충실도 diff,
  누름틀/필드 채우기, 템플릿 슬롯 `fill`/`slots`, 구조 `validate`, 문서 메타데이터 읽기/쓰기, MCP 서버.
- **PDF**는 텍스트 선택가능 벡터 출력이다(글리프 CIDFont 임베드 + 사용 글리프 서브셋, 래스터 이미지는
  XObject 임베드). 단 PDF/HTML/ODT는 단방향 내보내기이며 페이지 레이아웃(여백·단)까지 1:1 재현하지는 않는다.
...
- **의미 모델 한계**: 표·그림·구역·머리말/꼬리말·글상자·필드는 의미 파싱된다. 도형(사각형/타원/선)은
  렌더 시 회색 외곽선/형태로 그리고, 수식/차트/OLE은 외곽선+X placeholder로 표시한다(원형 보존은 되나
  의미 모델·포맷 간 합성은 없음). 누름틀/필드는 기존 이름의 값만 채울 수 있고 신규 필드 생성은 없다.
  메모(주석) 작성은 hwpx 출력 전용(실험적)이며 hwp 바이너리 출력에서는 생략된다.
...
| 명령 | 인자 / 플래그 | 설명 |
|---|---|---|
| `info <file>` | `--json` | 포맷/버전/속성/스트림 + 메타데이터(제목/지은이) 진단 |
| `cat <file>` | `--format plain\|markdown\|json\|html` (기본 `plain`), `--preview` | 본문 추출. `--preview`는 본문 파싱 없이 PrvText만 출력 |
| `convert <input> -o <output>` | `--to hwp\|hwpx\|md\|json\|html\|pdf\|odt`(생략 시 확장자 추론), `--strict`, `--preserve-layout`, `--embed-bin` | 포맷 변환. `--strict`는 보존 불가(드롭) 데이터 발견 시 비정상 종료(hwp/hwpx 대상). `--preserve-layout`는 무수정 왕복 전용 줄 배치 보존. `--embed-bin`은 JSON에 이미지 base64 임베드 |
| `render <input> -o <output>` | `--pages "1"\|"1-3"\|"all"`(기본 `all`), `--dpi <f64>`(기본 96), `--format png\|svg\|pdf`(생략 시 확장자 추론), `--font-dir <dir>`(반복) | 페이지를 PNG/SVG/PDF로 렌더 |
| `new -o <output>` | `--from <md\|json>`(생략 시 빈 문서), `--preset plain\|gongmun\|bogoseo`, `--title`, `--author`, `--subject` | markdown/JSON IR에서 새 문서 생성(메타데이터 포함) |
| `edit <input> -o <output>` | `--replace "찾기=>바꾸기"`, `--set-cell "표:행:열=값"`, `--set-field "이름=값"`, `--set-meta "키=값"`, `--add-memo "텍스트"`(모두 반복), `--verify` | 기존 문서 편집. `--set-meta` 키: title\|author\|subject\|keywords. `--add-memo`는 hwpx 출력 전용(실험적) |
| `slots <file>` | `--json` | `{{name}}` 텍스트 자리표시자(템플릿 슬롯) 목록 |
| `fill <input> -o <output>` | `--set "이름=값"`(반복), `--data <values.json>`, `--json` | hwpx 템플릿 `{{name}}` 치환(패키지·미리보기 보존, hwpx 전용) |
| `validate <file>` | `--json` | 구조 검증(mimetype·필수 엔트리·XML). 유효 시 종료코드 0 |
...
| 도구 | 필수 인자 | 기능 |
|---|---|---|
| `hwp_info` | `path` | 포맷/버전/속성/스트림 + 메타데이터 진단(JSON) |
...
| `hwp_convert` | `input`, `output` (`embed_bin`) | 포맷 변환(확장자로 결정 — html/pdf/odt 포함) |
...
| `hwp_slots` | `path` | `{{name}}` 텍스트 자리표시자 목록 |
| `hwp_fill` | `input`, `output`, `values{}` | hwpx 템플릿 `{{name}}` 치환(패키지 보존) |
| `hwp_validate` | `path` | 구조 검증 결과(`{valid, errors, warnings}`) |
...
| `hwp-convert` | IR ↔ markdown / HTML / ODT / JSON, 인메모리 편집(치환·셀·필드·메타·메모), 필드·슬롯 스캔 |
| `hwp-render` | IR → PDF / PNG / SVG 렌더러, 폰트 서브셋·이미지 임베드, 줄 배치 합성, 텍스트 셰이핑, 렌더 diff |
